    ...
    <div class="col-md-6">
        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
            ...
            <input class="form-control" name="title" type="text" id="title"
                value="{{ old('title', optional($storage)->title) }}" minlength="1" maxlength="50" required>
    ...
    <div class="col-md-6">
        <div class="form-group {{ $errors->has('mb_amount') ? 'has-error' : '' }}">
            ...
            <input class="form-control" name="mb_amount" type="number" id="mb_amount"
                value="{{ old('mb_amount', optional($storage)->mb_amount) }}" min="-2147483648" max="2147483647" required>
...
'Inactive' => 'Inactive'] as $key => $text)
                <option value="{{ $key }}" {{ old('status', optional($storage)->status) == $key ? 'selected' : '' }}>
                    {{ $text }}
                </option>
                @endforeach
            </select>
