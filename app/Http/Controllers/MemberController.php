...
class MemberController extends Controller
{
    ...
    {   
        ...
        $subscriptionPlan = SubscriptionPlan::where('user_id', Auth::user()->id)->first();
        ...
        if ($request->subscription_type == 'web') {
            $packageName = 'Ecommerce Website Installation Package';
        } else if ($request->subscription_type == 'android') {
            $packageName = 'Android App Installation Package';
        } else if ($request->subscription_type == 'ios') {
            $packageName = 'iOS App Installation Package';
        } else if ($request->subscription_type == 'web_plan') {
            $packageName = 'Ecommerce Website Subscription Plan';
            if (empty($subscriptionPlan) || $subscriptionPlan->web_status == 'Not Installed') {
                $isSubscriptionAllowed = false;
            }
        } else if ($request->subscription_type == 'android_plan') {
            $packageName = 'Android App Subscription Plan';
            if (empty($subscriptionPlan) || $subscriptionPlan->android_status == 'Not Installed') {
                $isSubscriptionAllowed = false;
            }
        } else if ($request->subscription_type == 'ios_plan') {
            $packageName = 'iOS App Subscription Plan';
            if (empty($subscriptionPlan) || $subscriptionPlan->ios_status == 'Not Installed') {
                $isSubscriptionAllowed = false;
            }
        } else if ($request->subscription_type == 'mobile_lifetime_plan') {
            $packageName = 'Android & iOS Package Single License';
        } else if ($request->subscription_type == 'web_lifetime_plan') {
            $packageName = 'Website Package Single License';
        } else if ($request->subscription_type == 'addon') {
            $package_total_price= $request->package_total_price;
            $option_ids=json_decode($request->option_ids);
            $package_detail=Package::where('package_code',$request->package_info)->first();
            $packageName= $package_detail->title;
            $store_id = $request->store_id;
        }else{
            $package_total_price= $request->package_total_price;
            $option_ids=json_decode($request->package_info);
            $package_detail=Package::where('package_code',$request->subscription_type)->first();
            $packageName= $package_detail->title;
        }
        ...
        if (!$isSubscriptionAllowed) {
            return redirect()
                ...
                ->with(
                    'error_message',
                    'You are not allowed to purchase subscriptions plans before install packages!'
                );
        }

        $theme = '';
        if (!empty($request->theme)) {
            $theme = optional(Product::find($request->theme))->title;
        }
    ...
    public function profile()
    {
        ...
        return view('members.settingsfiles.profile', compact('customer', 'countries'));
    }
    ...
    {
        $customer = User::findOrFail(Auth::user()->id);
        return view('members.settingsfiles.changepassword', compact('customer'));
    }
    public function userStores()
    {
        $setupPackages = Store::where('user_id',Auth::id())->get();
        return view('members.settingsfiles.mystores', compact('setupPackages'));
    }

    public function userStoreAddons($storeId)
    {
        $store=Store::find($storeId);
        $package_code=$store->package_code;
        $addons=null;
        $parent=Package::where('package_code',$package_code)->first();
        if(!empty($parent))
        {
            $parent_id=$parent->id;
            $addons=Package::where('parent_package',$parent_id)->first();
        }
        
        if(!empty($addons))
        {
             $view = view('members.settingsfiles.addons', compact('addons','store'));
             return ['status' => 'OK', 'html' => $view->render()];
        }else{
             return ['status' => 'ERROR', 'html' => 'No Addons for this package'];
        }
    ...
    public function userOrders()
    {
        $orders = Order::with('user')
            ...
            ->orderBy('id', 'DESC')
            ->get();
        return view('members.settingsfiles.myorders', compact('orders'));
    }
    ...
    protected function getPasswordData(Request $request, $user)
    {
        $rules = [
            ...
            'current_password' => ['required', 'min:6', 'max:20', function ($attribute, $value, $fail) use ($user) {
